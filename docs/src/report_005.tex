...
The LOSO correlation ($r=0.861$) is only 2.9\% below the pooled result
($r=0.887$). This small gap is expected: with 20 subjects sharing the
same mixing matrix $\mathbf{M}$, the cross-covariance
$\mathbf{R}_{YX}$ estimated from 19 subjects closely approximates the
true value. The held-out subject's noise realization is the main source
of prediction error.
