...
uv run python -m src.train --config configs/base.yaml 2>&1 | tee results/logs/closed_form.log

# Step 2: Train Model 1 (Linear Spatial Filter)
echo ""
echo "=== Step 2: Model 1 - Linear Spatial Filter ==="
uv run python -m src.train --config configs/model1_linear.yaml 2>&1 | tee results/logs/linear_spatial.log

# Step 3: Train Model 2 (FIR Filter)
echo ""
echo "=== Step 3: Model 2 - Spatio-Temporal FIR Filter ==="
uv run python -m src.train --config configs/model2_fir.yaml 2>&1 | tee results/logs/fir_filter.log

# Step 4: Train Model 3 (Convolutional Encoder)
echo ""
echo "=== Step 4: Model 3 - Convolutional Encoder ==="
uv run python -m src.train --config configs/model3_conv.yaml 2>&1 | tee results/logs/conv_encoder.log

# Step 5: Run ablation study
echo ""
echo "=== Step 5: Ablation study ==="
uv run python scripts/ablation.py 2>&1 | tee results/logs/ablation.log
