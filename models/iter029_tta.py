"""Iteration 029: Test-time augmentation (TTA).

Hypothesis: At test time, averaging predictions from multiple augmented views
of the input reduces noise and improves correlation. Augmentations that are
valid for EEG: time-reversal, small temporal shifts, channel noise.

We train a standard InstanceNorm+FIR model (best recipe), then wrap it in
a TTA module that averages predictions from:
1. Original input
2. Time-reversed input (reverse output)
3. +1 sample temporal shift
4. -1 sample temporal shift

Expected: +0.002-0.005 from prediction averaging.
"""
...
class TTAWrapper(nn.Module):
    """Test-time augmentation wrapper: averages predictions from augmented inputs."""
    ...
    def forward(self, x):
        ...
        # 2. Time-reversed
        x_rev = x.flip(dims=[-1])
        pred_rev = self.model(x_rev).flip(dims=[-1])
        preds.append(pred_rev)

        # 3. Shift +1 (pad left, crop right)
        x_shift_r = F.pad(x[:, :, :-1], (1, 0), mode='replicate')
        preds.append(self.model(x_shift_r))

        # 4. Shift -1 (pad right, crop left)
        x_shift_l = F.pad(x[:, :, 1:], (0, 1), mode='replicate')
        preds.append(self.model(x_shift_l))
...
def build_and_train(
    ...
    base_model = InstanceNormFIR(C_scalp, C_inear, filter_length=7).to(device)

    with torch.no_grad():
        base_model.fir.conv.weight.zero_()
        center = base_model.fir.filter_length // 2
        base_model.fir.conv.weight[:, :, center] = cf.W.float()

    # Train the base model (no TTA during training)
