"""Iteration 075: Target Noise Injection (DisturbValue).

Adding Gaussian noise to regression targets during training is equivalent to
Tikhonov regularization on the model weights (Bishop 1995). This acts as an
implicit regularizer without modifying the loss or architecture.

sigma=0.05 chosen to be ~5% of typical in-ear EEG std (~1.0 after z-scoring).

Hypothesis: Target noise will reduce overfitting to noisy EEG labels,
improving cross-subject generalization by ~0.002-0.005 r.
"""
...
def build_and_train(train_ds, val_ds, C_scalp, C_inear, device):
    ...
    print(f"Target noise sigma: {TARGET_NOISE_SIGMA}")
    ...
    for ep in range(1, 151):
        ...
        for x, y in tl:
            ...
            # --- Target noise injection (DisturbValue) ---
            y_noisy = y + TARGET_NOISE_SIGMA * torch.randn_like(y)

            # Mixup augmentation
            lam = np.random.beta(0.4, 0.4)
            idx = torch.randperm(x.shape[0], device=device)
            x = lam * x + (1 - lam) * x[idx]
            y_noisy = lam * y_noisy + (1 - lam) * y_noisy[idx]
            ...
            opt.zero_grad()
            loss = loss_fn(model(x), y_noisy)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()

        # Validate on clean targets
