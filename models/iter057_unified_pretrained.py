"""Iteration 057: Fine-tune unified pretrained encoder for Ear-SAAD.

Loads a TemporalEncoder pretrained via masked channel autoencoding on multiple
EEG datasets (scripts/pretrain_unified.py), then fine-tunes on scalp-to-in-ear.

Architecture:
  1. Pretrained TemporalEncoder (128-dim) applied per-channel -> (B, 46, 128)
  2. 2-block TransformerEncoder over 46 channel tokens (dim=128, 4 heads)
  3. Cross-attention decoder: 12 learnable queries attend to channel tokens
  4. Output projection: Linear(128, 256) per output channel
  5. CF skip connection for stable gradients

Training strategy:
  - Freeze encoder for first 30 epochs (train transformer + decoder only)
  - Unfreeze encoder at epoch 31 with 10x lower LR
...
# -- Pretrained encoder (exact copy from scripts/pretrain_unified.py) --------
...
class UnifiedPretrainedModel(nn.Module):
    """Fine-tune unified pretrained temporal encoder for scalp-to-in-ear.

    Pipeline:
        input (B, C_in, T)
        -> per-channel encode -> (B, C_in, 128)
        -> TransformerEncoder (channel interactions) -> (B, C_in, 128)
    ...
    def __init__(
        ...
        # 2. Channel-level transformer encoder (2 blocks, 4 heads, dim=128)
        ...
        #    Linear(128, 256) with intermediate expansion
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Channel transformer (2 blocks, 4 heads over 46 channel tokens)
        ...
        # Project to time dimension: Linear(128, 256)
...
def build_and_train(
    ...
    """Build and train unified-pretrained fine-tuning model."""

    T = train_ds.scalp.shape[2]  # window length (256 for broadband)
